...
import { Target, Compass, Award, CheckCircle, ArrowRight } from 'lucide-react';
    ...
    <div id="about-page" className="w-full pt-28 sm:pt-32 pb-20 bg-[#0A0A0A] text-slate-300">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16 sm:mb-24">
        <div className="text-center max-w-4xl mx-auto">
          ...
          <p className="text-base sm:text-xl text-slate-300 font-light leading-relaxed">
            {t('Translating time-tested military principles into high-performance commercial, industrial, and institutional excellence.')}
          </p>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            ...
            <div className="lg:col-span-4 border-t lg:border-t-0 lg:border-l border-[#222222] lg:pl-8 space-y-4 text-xs sm:text-sm text-slate-300">
              <div className="flex items-start gap-3">
                ...
                <span><strong className="text-white">{t('Veteran-Led:')}</strong> {t('Decades of elite military command and combat support discipline.')}</span>
              </div>
              <div className="flex items-start gap-3">
                <CheckCircle className="w-4 h-4 text-[#C5A059] shrink-0 mt-0.5" />
                <span><strong className="text-white">{t('Operational Focus:')}</strong> {t('Bridging boardroom strategy to frontline supervisor execution.')}</span>
              </div>
              <div className="flex items-start gap-3">
                <CheckCircle className="w-4 h-4 text-[#C5A059] shrink-0 mt-0.5" />
                <span><strong className="text-white">{t('High-Consequence Rigor:')}</strong> {t('Zero-defect mentality for infrastructure, defense, and logistics.')}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="bg-[#111111] py-20 border-y border-[#222222] mb-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <SectionHeader
            badge={t('The Strategic Challenge')}
            title={t('Closing the Strategy-Execution Chasm')}
            subtitle={t('Why traditional executive consulting fails at the frontline level—and how military operational discipline resolves it.')}
            align="center"
            theme="dark"
          />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-12">
            ...
            <div className="p-8 rounded-xl border border-[#222222] bg-[#141414] space-y-4">
              <div className="text-xs font-mono uppercase tracking-widest text-slate-500">
                {t('The Industry Problem')}
              </div>
              <h3 className="font-serif text-2xl font-bold text-white">
                {t('Strategy Without Operational Grounding')}
              </h3>
              <p className="text-sm text-slate-400 leading-relaxed font-light">
                {t('Enterprise organizations invest millions in strategic plans, only to see initiatives stall because frontline supervisors, shift leads, and field foremen lack the structured command tools, clear standards, and accountability frameworks required to execute.')}
              </p>
              <ul className="space-y-2 text-xs text-slate-400 pt-2 border-t border-[#222222]">
                <li>{t('• Diluted executive messaging across management tiers')}</li>
                <li>{t('• Frontline reluctance to enforce non-negotiable standards')}</li>
                <li>{t('• Lack of squad-level ownership and after-action review culture')}</li>
              </ul>
            </div>
            ...
            <div className="p-8 rounded-xl border-2 border-[#C5A059] bg-[#1A1A1A] space-y-4 shadow-xl">
              <div className="text-xs font-mono uppercase tracking-widest text-[#C5A059]">
                {t('The Elite Vanguard Solution')}
              </div>
              <h3 className="font-serif text-2xl font-bold text-white">
                {t('Military Precision Applied to Business')}
              </h3>
              <p className="text-sm text-slate-200 leading-relaxed font-light">
                {t('We embed time-tested military principles—direct example leadership, mission-type orders, rigorous standard operating procedures, and decentralized execution—into corporate teams, transforming supervisors into decisive, accountable leaders.')}
              </p>
              <ul className="space-y-2 text-xs text-[#C5A059] pt-2 border-t border-[#C5A059]/30">
                <li>{t("✓ Clear commander's intent translated to shift supervisors")}</li>
                <li>{t('✓ Culture of personal responsibility: Own the Outcome')}</li>
                <li>{t('✓ Decisive decision-making under high-pressure conditions')}</li>
              </ul>
            </div>
          </div>
        </div>
          ...
          <div className="p-10 rounded-xl border border-[#222222] bg-[#141414] text-slate-300 flex flex-col justify-between relative shadow-xl">
            <div>
              ...
              <h3 className="text-xl sm:text-2xl md:text-3xl font-serif font-bold text-white leading-snug mb-6">
                &ldquo;{t(COMPANY_INFO.vision)}&rdquo;
              </h3>
            </div>
            <div className="pt-6 border-t border-[#222222] text-xs uppercase tracking-widest text-[#C5A059] font-medium">
              {t('Nationwide Operational Leadership Benchmark')}
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-24">
        <SectionHeader
          badge={t('Guiding Principles')}
          title={t('Core Team Values')}
          subtitle={t('Our five non-negotiable operational principles guiding every engagement.')}
          align="center"
          theme="dark"
        />
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center pt-8 border-t border-[#222222]">
        <h2 className="text-3xl sm:text-4xl font-serif font-bold text-white mb-4">
          {t('Meet the Veterans Leading Elite Vanguard')}
        </h2>
        <p className="text-base text-slate-400 max-w-2xl mx-auto mb-8 font-light">
          {t('Discover the background and decorated service history of our co-founders across the military, infrastructure, and private equity sectors.')}
        </p>
